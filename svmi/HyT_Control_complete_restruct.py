...
class WorkThread(QThread):
    ...
    def tof_scan(self,units1, chno1, time1, units2, chno2, time2, \
                 freqs, pulsenature, checkboxvals, posnegvals, t2jumpfile,
                    scanchannel,relchannel,times):
                        

        for timing in times:
            output = ps.pulses(units1, chno1, time1, units2, \
                               chno2, time2, freqs, pulsenature, \
                               checkboxvals, posnegvals,scanchannel,\
                               relchannel,timing,t2jumpfile)
            plotseq.plotsequence(output,checkboxvals)
            print(' put in')
            input()
    ...
    def pulses(self,units1, chno1, time1, units2, chno2, \
               time2, freqs, pulsenature, checkboxvals, \
               posnegvals, scanchannel, relchannel, scantime, t2jumpfile):
        output=ps.pulses(units1, chno1, time1, units2, \
                         chno2, time2, freqs, pulsenature, \
                         checkboxvals, posnegvals, scanchannel, \
                         relchannel, scantime, t2jumpfile)
        return output
        

    def continuousTrig(self,units1, chno1, time1, units2, \
                       chno2, time2, freqs, pulsenature, \
                       checkboxvals, posnegvals, t2jumpfile):
        scantime = 'None'
        scanchannel = 'None'
        relchannel = 'None'
        output=ps.pulses(units1, chno1, time1, units2, \
...
class GuiThread(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    ...
    def OnListScan(self):
        ...
        try:
            ...
            self.ScanLog.append('Scanning custom list. First: %s %s, \
            Last: %s %s'%(scanlist[0],unit1,scanlist[-1],unit1))
    ...
    def start_working(self):
        self.TriggerLog.append('Welcome! You are using HyT-Control v.%s.\r'\
                                'Have fun!'%version)
        if devmode == 1:
            self.TriggerLog.append('nRuning in developer mode: No communication to devices.')
