...
class WorkThread(QThread):
    ...
    def tof_scan(self,scanchannel,relchannel,times):
		#~ scanchannel
		#~ relative channel
		#~ read board
		#~ change timings of channelno
			#~ calculate pulses
        ...
        for timing in times:
            ps.pulses(units1, chno1, time1, units2, chno2, time2, freqs, pulsenature, checkboxvals, posnegvals,scanchannel,relchannel,timing,t2jumpfile)
        return
        
    def PlotSeq(self):
        self.TrigLog.emit('Plot pulse sequence.')
        print(self.readboard.emit())
        
        #~ scanchannel = 'None'
        #~ relchannel = 'None'
        #~ scantime = 'None'
        #~ self.TrigLog.emit('Calculating sequence...')
...
class GuiThread(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    def __init__(self):
        ...
        self.LoadBoardButton.clicked.connect(self.OnBOOM)
    ...
    def OnBOOM(self):
        ...
        ps.pulses(units1, chno1, time1, units2, chno2, time2, freqs, pulsenature, checkboxvals, posnegvals,scanchannel,relchannel,scantime,t2jumpfile)
