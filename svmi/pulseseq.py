...
def lcm(a):
	'''
			find the lowest common multiple of items in a list
			the current scheme works only for integer number of repetition rate
	'''
	lcm = a[0]
	for i in a[1:]:
...
def pulses(units1, chno1, time1, units2, chno2, time2, freqs, pulsenature, checkboxvals, posnegvals, scanchannel, relchannel, scantime, t2jumpfile):
	...
	chno=18
	ch_done=[]
	ch_list=[i for i in range(lastindex+1)] # [0,1,2,...,lastindex]

	durations={} # define dict
	events={}
	...
	while tottime<pulsetime:
		...
		if checkboxvals[index]==True:
			...
			# print(channels,events,durations)
		else:
	...
	'''
			...
			events are fine upto here
	'''
		
	
	#~ print(events)
	# problem is here:
	# there are timings with the same value (start right after the stop)
	# in this case sort doesn't work properly
	...
	keyindexes = np.argsort(list(events.values()))
